 ...
 * Note to LetterN: Make this use overlay magic instead of normal icon for m o d u l a r i t y
 */

/obj/structure/medkit_cabinet
	name = "Medkit Cabinet"
	desc = "A small wall mounted cabinet designed to hold a medical kit."
	icon = 'modular_hyperstation/icons/obj/wallmounts.dmi'
	icon_state = "medkit_closed"
	anchored = TRUE
	density = FALSE
	max_integrity = 200
	integrity_failure = 50
	var/obj/item/storage/firstaid/regular/stored_medkit //any medkits
	var/opened = FALSE

//Initialize() Calls when SS is starting up. It's controled by the MC rather than BYOND, unlike New, BYOND controls new
...
/obj/structure/medkit_cabinet/contents_explosion(severity, target)
	if(stored_medkit)
		stored_medkit.ex_act(severity, target) //this pushes the bomb act effect to simulate in the box
...
/obj/structure/medkit_cabinet/attackby(obj/item/I, mob/user, params)
	...
	if(iscyborg(user) || isalien(user))
		return
	if(istype(I, /obj/item/storage/firstaid/regular))
...
/obj/item/wallframe/medkit_cabinet
	...
	icon = 'modular_hyperstation/icons/obj/wallframe.dmi'
	icon_state = "medkit"
