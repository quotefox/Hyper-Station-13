//Code ported from Lobotomy Corp 13, and adapted for Hyper's old shitcode.
//I sincerely apologize that your wonderful sprites and work are in our setting.
//Damage values and health adjusted because furries aren't robust.

/mob/living/simple_animal/hostile/ordeal/green_bot
	name = "doubt"
	desc = "A slim robot with a spear in place of its hand."
	icon = 'hyperstation/icons/mobs/32x48.dmi'
	...
	attacktext = "stabs"
	attack_sound = 'hyperstation/sound/effects/ordeals/green/stab.ogg'
	...
	if(.)
		...
		if(TH.health < 0)
			...
			for(var/i = 1 to 7)
				...
				/* Rework to weave damage, once done
			...
			if(!targets_from.Adjacent(TH) || QDELETED(TH))
				finishing = FALSE
				return
			playsound(get_turf(src), 'hyperstation/sound/effects/ordeals/green/final_stab.ogg', 50, 1)
			TH.gib()
			/*
			for(var/mob/living/carbon/human/H in view(7, get_turf(src)))
...
/mob/living/simple_animal/hostile/ordeal/green_bot_big
	...
	attacktext = "slices"
	attack_sound = 'hyperstation/sound/effects/ordeals/green/saw.ogg'
	ranged = 1
	rapid = 8
	rapid_fire_delay = 3
	projectiletype = /obj/item/projectile/bullet/latob
	projectilesound = 'hyperstation/sound/effects/ordeals/green/fire.ogg'
	death_sound = 'hyperstation/sound/effects/ordeals/green/noon_dead.ogg'
	damage_coeff = list(BRUTE = 1)
	/*
		...
		for(var/i = 1 to 4)
			...
			for(var/mob/living/L in T.contents)
				if(faction_check_mob(L))
					continue
				L.apply_damage(8, BRUTE, null, L.run_armor_check(null, BRUTE), spread_damage = TRUE)
			SLEEP_CHECK_DEATH(1)

/mob/living/simple_animal/hostile/ordeal/green_bot_big/proc/StartReloading()
	reloading = TRUE
	icon_state = "green_bot_reload"
	playsound(get_turf(src), 'hyperstation/sound/effects/ordeals/green/cooldown.ogg', 50, FALSE)
	for(var/i = 1 to 8)
...
/mob/living/simple_animal/hostile/ordeal/green_dusk
	...
	icon = 'hyperstation/icons/mobs/64x48.dmi'
