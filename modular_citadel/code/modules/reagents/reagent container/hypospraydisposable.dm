...
//A disposable hypospray. Duplicated from orginal hypospray code to avoid fucking it up.
/obj/item/hypospray/disposable
	name = "hypospray mk.II"
	icon = 'modular_citadel/icons/obj/hypospraymkii.dmi'
	icon_state = "hypo2"
	desc = "A cheaply made, nonreusable version of the MK.II hypospray. Once a hypovial is loaded it cannot be removed without breaking the hypospray."
...
/obj/item/hypospray/disposable/Initialize()
	. = ..()
	if(!spawnwithvial)
		update_icon()
		return
...
/obj/item/hypospray/disposable/update_icon()
	..()
...
/obj/item/hypospray/disposable/examine(mob/user)
	. = ..()
...
/obj/item/hypospray/disposable/proc/unload_hypo(obj/item/I, mob/user)
	if((istype(I, /obj/item/reagent_containers/glass/bottle/vial)))
		to_chat(user, "<span class='notice'>You can't remove the [vial] from the [src].</span>")

/obj/item/hypospray/disposable/attackby(obj/item/I, mob/living/user)
	...
	if((istype(I, /obj/item/reagent_containers/glass/bottle/vial)))
		...
		if(!user.transferItemToLoc(V,src))
			return FALSE
		vial = V
		user.visible_message("<span class='notice'>[user] has loaded a vial into [src].</span>","<span class='notice'>You have loaded [vial] into [src] locking it into place.</span>")
		update_icon()
...
/obj/item/hypospray/disposable/AltClick(mob/user)
	if(vial)
		vial.attack_self(user)

// Gunna allow this for now, still really don't approve - Pooj
/obj/item/hypospray/disposable/emag_act(mob/user)
...
/obj/item/hypospray/disposable/attack_hand(mob/user)
	. = ..() //Don't bother changing this or removing it from containers will break.

/obj/item/hypospray/disposable/attack(obj/item/I, mob/user, params)
	return

/obj/item/hypospray/disposable/afterattack(atom/target, mob/user, proximity)
	if(!vial)
		return
...
/obj/item/hypospray/disposable/attack_self(mob/living/user)
	if(user)
...
/obj/item/hypospray/disposable/verb/modes()
