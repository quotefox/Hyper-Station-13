...
/datum/status_effect/chem/enthrall/on_apply()
	...
	if(!E)
		message_admins("WARNING: FermiChem: No master found in thrall, did you bus in the status? You need to set up the vars manually in the chem if it's not reacted/bussed. Someone set up the reaction/status proc incorrectly if not (Don't use donor blood).")
		owner.remove_status_effect(src)
