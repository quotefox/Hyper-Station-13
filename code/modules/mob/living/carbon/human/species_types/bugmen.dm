/datum/species/insect
	...
	species_traits = list(LIPS,EYECOLOR,HAIR,FACEHAIR,MUTCOLORS,HORNCOLOR)
	inherent_biotypes = MOB_ORGANIC|MOB_HUMANOID|MOB_BUG
	mutant_bodyparts = list("mam_ears", "mam_snout", "mam_tail", "taur", "insect_wings", "mam_snouts", "insect_fluff","horns","moth_markings")
	default_features = list("mcolor" = "FFF","mcolor2" = "FFF","mcolor3" = "FFF", "mam_tail" = "None", "mam_ears" = "None","moth_markings" = "None",
							"insect_wings" = "Plain", "insect_fluff" = "Plain", "mam_snouts" = "None", "taur" = "None","horns" = "None")
	attack_verb = "slash"
...
/datum/species/insect/on_species_gain(mob/living/carbon/C)
	...
	if(ishuman(C))
		...
		if(!H.dna.features["insect_wings"])
			H.dna.features["insect_wings"] = "[(H.client && H.client.prefs && LAZYLEN(H.client.prefs.features) && H.client.prefs.features["insect_wings"]) ? H.client.prefs.features["insect_wings"] : "Plain"]"
...
/datum/species/insect/handle_fire(mob/living/carbon/human/H, no_protection = FALSE)
	..()
	if(H.dna.features["insect_wings"] != "Burnt Off" && H.bodytemperature >= 800 && H.fire_stacks > 0) //do not go into the extremely hot light. you will not survive
...
/datum/species/insect/space_move(mob/living/carbon/human/H)
	. = ..()
	if(H.loc && !isspaceturf(H.loc) && H.dna.features["insect_wings"] != "Burnt Off")
