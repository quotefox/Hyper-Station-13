...
/datum/preferences/proc/update_preferences(current_version, savefile/S)
	...
	if(current_version < 24)
		if(features["legs"] == "Normal Legs")
			features["legs"] = "Plantigrade"
			WRITE_FILE(S["feature_lizard_legs"], features["legs"])
		var/species_id
		var/horns_reset = TRUE
		...
		if(species_id)
			...
			if(species_id != "lizard")
				horns_reset = FALSE

			WRITE_FILE(S["species"], species_id)

		if(horns_reset)
			features["horns"] = "None"	//Because you still have it, you just cant select it
			WRITE_FILE(S["feature_lizard_horns"], features["horns"])
		horn_color = "865"	//Gonna be null the first time. Don't want it looking weird, so here's a hack
	return
...
/datum/preferences/proc/load_character(slot)
	...
	S["species"]			>> species_id
	if(species_id)
		if(species_id == "avian" || species_id == "aquatic")	//This is before the character is updated, from version 24. If you feel like this isnt needed anymore, remove this
			species_id = "mammal"
		else if(species_id == "moth")	//Same here
			species_id = "insect"
	...
	S["hide_ckey"]				>> hide_ckey	//Despite this looking like a preference, this can be different with each character
...
#ifndef TESTING	//if not testing
	WRITE_FILE(S["version"]			, SAVEFILE_VERSION_MAX)	//load_character will sanitize any bad data, so assume up-to-date.)
#else
	var/version = S["version"]
	testing("SAVEFILE: Version [version == SAVEFILE_VERSION_MAX ? "is up-to-date with" : "[version] does not match"] the current version [SAVEFILE_VERSION_MAX]")
	if(version != SAVEFILE_VERSION_MAX)
		testing("SAVEFILE: The version does not update while in testing mode.")
#endif
	//Character
