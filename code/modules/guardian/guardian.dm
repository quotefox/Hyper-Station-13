...
/mob/living/simple_animal/hostile/guardian/proc/setup_barriers()
	...
	for(var/turf/T in getline(locate(sx - range, sy + range + 1, sz), locate(sx + range, sy + range + 1, sz)))
		barrier_images += image('icons/effects/effects2.dmi', T, "barrier", ABOVE_LIGHTING_LAYER, SOUTH)
	for(var/turf/T in getline(locate(sx - range, sy - range - 1, sz), locate(sx + range, sy - range - 1, sz)))
		barrier_images += image('icons/effects/effects2.dmi', T, "barrier", ABOVE_LIGHTING_LAYER, NORTH)
	for(var/turf/T in getline(locate(sx - range - 1, sy - range, sz), locate(sx - range - 1, sy + range, sz)))
		barrier_images += image('icons/effects/effects2.dmi', T, "barrier", ABOVE_LIGHTING_LAYER, EAST)
	for(var/turf/T in getline(locate(sx + range + 1, sy - range, sz), locate(sx + range + 1, sy + range, sz)))
		barrier_images += image('icons/effects/effects2.dmi', T, "barrier", ABOVE_LIGHTING_LAYER, WEST)
	barrier_images += image('icons/effects/effects2.dmi', locate(sx - range - 1 , sy + range + 1, sz), "barrier", ABOVE_LIGHTING_LAYER, SOUTHEAST)
	barrier_images += image('icons/effects/effects2.dmi', locate(sx + range + 1, sy + range + 1, sz), "barrier", ABOVE_LIGHTING_LAYER, SOUTHWEST)
	barrier_images += image('icons/effects/effects2.dmi', locate(sx + range + 1, sy - range - 1, sz), "barrier", ABOVE_LIGHTING_LAYER, NORTHWEST)
	barrier_images += image('icons/effects/effects2.dmi', locate(sx - range - 1, sy - range - 1, sz), "barrier", ABOVE_LIGHTING_LAYER, NORTHEAST)
	for(var/image/I in barrier_images)
